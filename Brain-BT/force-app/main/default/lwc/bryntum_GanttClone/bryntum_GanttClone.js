...
export default class Gantt_component extends NavigationMixin(LightningElement){
    ...
    @wire(pickListValueDynamically, {customObjInfo: {'sobjectType' : 'buildertek__Project_Task__c'},
    selectPicklistApi: 'buildertek__Phase__c'}) selectTargetValues;
    
    @wire(pickListValueDynamically, {customObjInfo: {'sobjectType' : 'buildertek__Project_Task__c'},
    selectPicklistApi: 'buildertek__Type__c'}) selectTargetTypeValues;
    ...
    getComment(){
        var that = this
        this.isLoaded = true;
        getAllNotes({schId: this.taskRecordId}).then(function(response){
            console.log(response)
    ...
    saveComment(){
        ...
        if(recId && newNotes){
            ...
            if(commentList.length){
                addNotesCommentToRecord({schItem : this.taskRecordId,notes: commentList}).then(function(response){
                    console.log(response);
                    if(response == 'Success'){
    ...
    handleAccountSelection(event){
        ...
        }else if(event.detail.fieldNameapi == 'buildertek__Contractor_Resource__c'){
            this.newTaskRecordCreate['buildertek__Contractor_Resource__c'] = event.detail.Id;
    ...
    saveSelectedContact(){
        ...
        if(!this.taskRecordId.includes('_generated')){
            saveResourceForRecord({taskId: this.taskRecordId,resourceId: this.selectedResourceContact,resourceApiName: this.selectedContactApiName}).then(function(response){
    ...
    changeInputVal(e){
        ...
        if(e.currentTarget.dataset.inputname == 'buildertek__Dependency__c'){
            this.newTaskRecordCreate[e.currentTarget.dataset.inputname] = this.template.querySelectorAll('lightning-input-field')[1].value
        }else if(e.currentTarget.dataset.inputname == 'buildertek__Resource__c'){
            this.newTaskRecordCreate[e.currentTarget.dataset.inputname] = this.template.querySelectorAll('lightning-input-field')[2].value
        }else if(e.currentTarget.dataset.inputname == 'buildertek__Contractor__c'){
            this.newTaskRecordCreate[e.currentTarget.dataset.inputname] = this.template.querySelectorAll('lightning-input-field')[3].value
        }else if(e.currentTarget.dataset.inputname == 'buildertek__Contractor_Resource__c'){
            this.newTaskRecordCreate[e.currentTarget.dataset.inputname] = this.template.querySelectorAll('lightning-input-field')[4].value
        }else if(e.target){
            if(e.currentTarget.dataset.inputname == 'buildertek__Start__c'){
                var stDate = new Date(Number(e.target.value.split('-')[0]),Number(e.target.value.split('-')[1])-1,Number(e.target.value.split('-')[2]))
                var endDate ;
                /* if(stDate.getDay() == 6){
                ...
                var duration = Number(this.newTaskRecordCreate['buildertek__Duration__c'])
                if(duration){
                    ...
                    var thatThis = this;
                    getEndDate({sDate: e.target.value,eDate : this.newTaskRecordCreate['buildertek__Finish__c'],duration: duration}).then(function(response){
                        ...
                        if(thatThis.template.querySelectorAll('lightning-input')){
                            if(thatThis.template.querySelectorAll('lightning-input')[3].label == 'End Date'){
                                thatThis.template.querySelectorAll('lightning-input')[3].value = thatThis.newTaskRecordCreate['buildertek__Finish__c']
                            }
                        }
                    })
            ...
            }else if(e.currentTarget.dataset.inputname == 'buildertek__Duration__c'){
                var stdateVal = this.newTaskRecordCreate['buildertek__Start__c']
                var stDate = new Date(Number(stdateVal.split('-')[0]),Number(stdateVal.split('-')[1])-1,Number(stdateVal.split('-')[2]))
                var endDate ;
                /* if(stDate.getDay() == 6){
                    stDate.setDate(stDate.getDate() +2);
                    console.log(stDate)
                ...
                if(duration){
                   ...
                   // endDate = new Date(endDate)
                    ...
                    //endDate.setDate(endDate.getDate() +days);
                    var thatThis = this;
                    getEndDate({sDate: this.newTaskRecordCreate['buildertek__Start__c'],eDate : this.newTaskRecordCreate['buildertek__Finish__c'],duration: duration}).then(function(response){
                        var newTaskeDate = response//.toLocaleDateString().split('/');
                        thatThis.newTaskRecordCreate['buildertek__Finish__c'] =  newTaskeDate// newTaskeDate[2]+'-'+newTaskeDate[1]+'-'+newTaskeDate[0];
                        if(thatThis.template.querySelectorAll('lightning-input')[3].label == 'End Date'){
                            thatThis.template.querySelectorAll('lightning-input')[3].value = thatThis.newTaskRecordCreate['buildertek__Finish__c']
                        }
                    })
    ...
    getRecordData(recID,recData){
        if(recID){
            this.isLoaded = true;
            var recThis = this;
            getTask({taskId: recID}).then(function(response){
                console.log(response)
               ...
               // recThis.selectedPredecessor = response['buildertek__Dependency__c']
                ...
                recThis.newTaskRecordCreate['buildertek__Dependency__c'] = recData.predecessor;
                if(recData.internalresource != response['buildertek__Completion__c'])
                recThis.newTaskRecordCreate['buildertek__Resource__c'] = recData.internalresource;
                if(recData.contractorresource != response['buildertek__Completion__c'])
                recThis.newTaskRecordCreate['buildertek__Contractor_Resource__c'] = recData.contractorresource
                if(recData.contractoracc != response['buildertek__Completion__c'])
                recThis.newTaskRecordCreate['buildertek__Contractor__c'] = recData.contractoracc
               ...
               // console.log(recThis.selectedPredecessor)
                ...
                recThis.newTaskRecordCreate['buildertek__Completion__c'] = recData.percentDone;
    ...
    droprow(e){
       ...
       /*  var taskList = this.GanttVar.tasks
        ...
        if(data.indexOf('_') > -1 && tar.indexOf('_') > -1 && this.scheduleItemsData.length){
            ...
            if(index1>index2) {
                ...
                if(taskListNew[index2]){
                    ...
                    var temp = fromatList.splice(dropLen,taskListNew[index2].value.length)
        ...
        let recordsMap = new Map();
          for (var i in taskList) {
            if(taskList[i].buildertek__Phase__c){
              ...
              recordsMap.get(taskList[i].buildertek__Phase__c).push(JSON.parse(JSON.stringify(taskList[i])));
            }else{
    ...
    callinsertUpdateTaskList(taskData){
        var that = this;
        this.isLoaded = true;
        insertUpdateTaskList({taskJSON: JSON.stringify(taskData),isUpdate: true}).then(function(response){
            console.log(response)
    ...
    addNewTask(record){
        ...
        if(record){
           ...
           // this.newTaskRecordCreate['buildertek__Order__c'] = record._data.order;
            if(record._data.phase){
                this.newTaskRecordCreate['buildertek__Phase__c'] = record._data.phase != 'null' ? record._data.phase: ''
        ...
        this.newTaskRecordCreate['buildertek__Start__c'] = dt.getFullYear()+'-'+Number(dt.getMonth()+1)+'-'+dt.getDate();
        this.newTaskRecordCreate['buildertek__Duration__c'] = 1;
        this.newTaskRecordCreate['buildertek__Finish__c'] = this.newTaskRecordCreate['buildertek__Start__c']
    ...
    deleteRecord(){
        ...
        if(task){
            ...
            deleteTasks({taskId: deleteId,type: task._data.type,ScheduleId : this.scheduleData.Id}).then(function(response){
                console.log(response)
            ...
            scheduleid : this.recordId            
              ...
              for (var i in scheduleItemsListClone) {
                if(scheduleItemsListClone[i].buildertek__Phase__c){
                  ...
                  recordsMap.get(scheduleItemsListClone[i].buildertek__Phase__c).push(JSON.parse(JSON.stringify(scheduleItemsListClone[i])));
                }else{
                  if (!recordsMap.has('null')) {
                      recordsMap.set('null', []);
                  }
                  recordsMap.get('null').push(JSON.parse(JSON.stringify(scheduleItemsListClone[i])));
                } 
              }
              var result = Array.from(recordsMap.entries());
              var groupData = [];
    ...
    openMasterSchedule() {
        const urlWithParameters = '/lightning/cmp/buildertek__ImportMasterSchedule?buildertek__RecordId='+this.scheduleData.Id+'&buildertek__isFromNewGantt='+true;
    ...
    openScheduleLines(){
       const urlWithParameters = '/lightning/cmp/buildertek__ImportScheduleLines?buildertek__RecordId='+this.scheduleData.Id+'&buildertek__isFromNewGantt='+true;
        console.log('urlWithParameters...'+urlWithParameters);
        this[NavigationMixin.Navigate]({
            type: 'standard__webPage',
            attributes: {
    ...
    saveeditRecord(event){
      ...
      if(event.currentTarget.name =='saveNew'){
        ...
        if(this.newTaskRecordCreate.Name){
            ...
            insertUpdateTask({taskFields: JSON.stringify(this.newTaskRecordCreate),isUpdate: isNotInsert,scheduleId : schId}).then(response => {
                console.log(response)
                const filterChangeEvent = new CustomEvent('filterchange', {
                    detail: { 'message' :'refresh page' },
                });
    ...
    createGantt() {
        var GanttToolbar
        if(this.userProfileName != "System Administrator" && this.userProfileName != 'Project Coordination'){
        ...
        const gantt = new bryntum.gantt.Gantt({
            ...
            columns: [
                ...
                { 
                    ...
                    renderer : function(record){
                        ...
                        if(record.record._data.iconCls == 'b-fa b-fa-arrow-left indentTrue'){
                            ...
                            if(record.record._data.duration >= 1 && record.record._data.type == "Task" && record.record._data.name != 'Milestone Complete'){
                               ...
                               // if(record.record._data.startDate){
                                   /*  var start = new Date(record.record._data.startDate.getTime())
                                    ...
                                    console.log('testendDate-->',record.record._data.name,record.record._data.duration,endDate)

                                    
                                    
                                    var eDateafter = endDate;
                                    console.log('eDateafter --> '+eDateafter);
                                console.log('testendDate-->',record.record._data.duration,endDate)
                                //console.log(endDate)
                                return months[endDate.getMonth()]+' '+Number(endDate.getDate())+', '+endDate.getFullYear();
                            }else if(record.record._data.duration >= 1 && record.record._data.type != "Task" && record.record._data.name != 'Milestone Complete'){
                                endDate = new Date(record.value);
                                endDate.setDate(endDate.getDate() - 1)
                                endDate = new Date(endDate)
                                return months[endDate.getMonth()]+' '+Number(endDate.getDate())+', '+endDate.getFullYear();
                            }else if(record.record._data.duration > -2 && record.record._data.type == "Task" && record.record._data.name != 'Milestone Complete'){
                                endDate = new Date(record.value);
                                endDate.setDate(endDate.getDate() - 1)
                                endDate = new Date(endDate)
                                return months[endDate.getMonth()]+' '+Number(endDate.getDate())+', '+endDate.getFullYear();
                ...
                /* { 
                 ...
                 { 
                    ...
                    editor : false,
                    renderer : function (record) {
                        if(record.record._data.type == 'Task'  && record.record._data.name != 'Milestone Complete'){
            ...
            listeners : {
                beforeCellEditStart : ({ editorContext }) => {
                    if(editorContext.column.field !== 'percentDone' || editorContext.record.isLeaf){
                        if(editorContext.column.field == "endDate"){
                            console.log(editorContext)
                            var StartString = editorContext.record._data.startDate.getFullYear()+'-'+Number(editorContext.record._data.startDate.getMonth()+1)+'-'+editorContext.record._data.startDate.getDate();
                            var that = this;
                            console.log(editorContext.value)
                                var start = new Date(editorContext.record._data.startDate.getTime())
                ...
                beforeFinishCellEdit : ({ editorContext }) => {
                    if(editorContext.column.field !== 'percentDone' || editorContext.record.isLeaf){
                        if(editorContext.column.field == "endDate"){
                            console.log(editorContext)
                            var StartString = editorContext.record._data.startDate.getFullYear()+'-'+Number(editorContext.record._data.startDate.getMonth()+1)+'-'+editorContext.record._data.startDate.getDate();
                            var endString =  editorContext.value.getFullYear()+'-'+Number(editorContext.value.getMonth()+1)+'-'+editorContext.value.getDate();
                            console.log(this.template.querySelector('.b-cell-editor'))

                             if(this.template.querySelector('.b-cell-editor')){
                                console.log(editorContext.value)
                                console.log(this.template.querySelector(".b-cell-editor").getElementsByTagName('input')[0].value)
                                var dateStringVal = this.template.querySelector(".b-cell-editor").getElementsByTagName('input')[0].value
                                //Oct 8, 2021 sample value
                                var months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
                                var endDateYearVal = dateStringVal.split(', ')[1]
                                var endDateMonthVal = months.indexOf(dateStringVal.split(', ')[0].split(' ')[0])
                                var endDateDateVal = dateStringVal.split(', ')[0].split(' ')[1]
                                
                                var dt = new Date(endDateYearVal,endDateMonthVal,endDateDateVal)
                                ...
                                for(var i=0;i<=diffDays;i++){
                                    ...
                                    if(new Date(stDate).getDay() != 0 && new Date(stDate).getDay() != 6){
                                        duration = duration+1;
                                    }
                ...
                beforerendertask : ({source}) =>{
                    if(this.GanttVar){
                        if(this.GanttVar.subGrids){
                            if(this.GanttVar.subGrids['normal'].collapsed == true || this.GanttVar.subGrids['normal'].collapsed == false){
                    ...
                    if(source._focusedCell){
                        ...
                        if(source._focusedCell.columnId.indexOf('endDate') > -1){
                            ...
                            console.log(source._selectedCell.element.innerText)
                                ...
                                if(this.template.querySelector(".b-cell-editor")){
                                    ...
                                    console.log(dt)
                                    var duration = 0;
                                    const date1 = new Date(source._selectedRecordCollection._values[0]._data.startDate)
                                    const date2 = new Date(dt)
                                    const diffTime = Math.abs(date2 - date1);
                                    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
                                    console.log(diffTime + " milliseconds");
                        ...
                        }else if(source._focusedCell.columnId.indexOf('fullDuration') > -1){
                ...
                if(taskRecord._data.endDate){
                    ...
                    if(eDate.getTime() < dat.getTime() && taskRecord.percentDone < 100) {
                        ...
                        if(renderData.taskRecord.cls.contains('pastDueDatesText')){
                            ...
                            renderData.row.elements.locked.classList.remove('pastDueDatesText')
                        }
                    ...
                    if(taskRecord.endDate && recEdate.getTime() < Date.now() && taskRecord.percentDone < 100) {
                        // if (taskRecord.endDate.getTime() < Date.now() && taskRecord.percentDone < 100) {
                             // fade out tasks in the past
                             console.log(taskRecord)
                             console.log(taskRecord.name)
       ...
       // gantt.generateDataset();
        if(this.template.querySelector('.projectname')){
            if(this.scheduleData.buildertek__Project__c){
                this.template.querySelector('.projectname').getElementsByTagName('h1')[0].innerHTML =  `Project - <a href='/`+this.scheduleData.buildertek__Project__c+`' style='color:white;'>`+this.scheduleData.buildertek__Project__r.Name+`</a>` 
            }else{
                 this.template.querySelector('.projectname').getElementsByTagName('h1')[0].innerText = 'Project - '
            }
        }
        ...
        gantt.addListener('cellClick', (event) =>  {
            ...
            }else if(event.record._data.startDate && !event.target.classList.contains('b-sch-timeaxis-cell')){
                this.GanttVar.scrollToDate(event.record._data.startDate, { block : 'center', animate : 500 });
            }
            ...
            if(event.column.data.text == 'Contractor Resource'){
                if(event.target.id == 'editcontractorResource'){
                    ...
                    if(event.target.dataset.resource){
                        ...
                        this.selectedResourceContact = event.record._data.contractorresource
                    }
        ...
        setTimeout(() =>  {
            ...
            if(this.userProfileName != "System Administrator" && this.userProfileName != 'Project Coordination'){
                if(this.template.querySelector('.importMasterBtn')){
                    this.template.querySelector('.importMasterBtn').className += ' hideBtn'
                }
                if(this.template.querySelector('.importScheduleLinesBtn')){
                    this.template.querySelector('.importScheduleLinesBtn').className += ' hideBtn'
                }
            }
        }, 1000);
